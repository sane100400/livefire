...
실사용 명령은 `scripts/gitctf.py` 하나를 기본 진입점으로 봅니다.
참가자는 `login/check/push`, 운영자는 `admin status/preflight/round/bundle`을 씁니다.
...
PoC runner 계약을 디버깅할 때만 선택적으로 확인:

```bash
python scripts/gitctf.py check --repo web_service --vuln 1 --poc poc.py
```
...
| `agent_sdk/` | 호환용 공통 SDK입니다. 직접 써도 되고, OpenAI/OpenRouter 호환 wrapper와 `/agent/*` API를 직접 호출해도 됩니다. |
...
3. 공격 에이전트가 target repo snapshot을 받고, coordinator OpenRouter 호환 wrapper로 허용 모델을 호출합니다.
4. 공격 에이전트가 `/attack`으로 탐색하고, 재현 가능한 PoC 소스를 `/pocs`로 제출합니다.
...
> "이건 AI 에이전트가 직접 웹 서비스를 공격하고 방어하는 CTF 운영 플랫폼입니다. 참가팀은 취약한 서비스를 만들고, 운영 서버는 flag 주입, 에이전트 실행, PoC 검증, 점수 계산을 자동화합니다. 중요한 점은 공격과 방어 결과물이 모두 공식 run token과 LLM 감사 로그에 묶여서, 사람이 몰래 제출한 산출물과 구분된다는 것입니다."
...
- agent 코드 구조를 바꿨다면 `python scripts/gitctf.py agent doctor --mode attack`과 `--mode defense`로 runner 호환성을 확인해야 합니다.
- 사전검증 완료 후 추가 push, 인증 없는 git clone/fetch, `RUNNER_SECRET` 없는 agent run 생성은 서버에서 거부되어야 합니다.
